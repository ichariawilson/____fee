...
namespace Microsoft.Fee.WebMVC.Services
{
    public class BasketService : IBasketService
    {
        ...
        private readonly string _applyUrl;
        ...
        public BasketService(HttpClient httpClient, IOptions<AppSettings> settings, ILogger<BasketService> logger)
        {
            ...
            _basketByPassUrl = $"{_settings.Value.ApplyUrl}/b/api/v1/basket";
            _applyUrl = $"{_settings.Value.ApplyUrl}/api/v1";
        }
        ...
        public async Task<Basket> SetSlots(ApplicationUser user, Dictionary<string, int> quantities)
        {
            var uri = API.Apply.UpdateBasketItem(_applyUrl);
        ...
        public async Task<Application> GetApplicationDraft(string basketId)
        {
            var uri = API.Apply.GetApplicationDraft(_applyUrl, basketId);
        ...
        public async Task AddItemToBasket(ApplicationUser user, int scholarshipItemId)
        {
            var uri = API.Apply.AddItemToBasket(_applyUrl);
